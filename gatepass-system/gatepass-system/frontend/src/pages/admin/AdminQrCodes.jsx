  ...
  return (
    <div className="flex bg-ink-900/[0.02]">
      ...
      <main className="h-screen flex-1 overflow-y-auto px-8 py-6">
        ...
        {urls && (urls.employeeUrl.includes("localhost") || urls.visitorUrl.includes("localhost")) && (
          <div className="mt-6 max-w-2xl rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            These QR codes currently point to <code className="rounded bg-white/60 px-1">localhost</code> — the backend's{" "}
            <code className="rounded bg-white/60 px-1">FRONTEND_URL</code> environment variable hasn't been set to your real
            domain yet. Update it and restart the backend before printing these for production use — codes generated
            against localhost won't work on visitors' phones.
          </div>
        )}
      </main>
    </div>
