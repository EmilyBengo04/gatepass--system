...
router.post("/signin", async (req, res) => {
  const { name, phone, email, host_employee_id, purpose, assets = [], client_ref } = req.body;
  ...
  try {
    // Idempotency check for retried offline-queue submissions — see the
    // matching comment in employee.routes.js for why this matters.
    if (client_ref) {
      ...
      if (existing[0]) {
        client.release();
        return res.status(200).json({ visitLogId: existing[0].id, timeIn: existing[0].time_in, alreadyProcessed: true, notifyStatus: "skipped" });
      }
    }
    ...
    const { rows: visitRows } = await client.query(
      `INSERT INTO visit_logs (visitor_type, visitor_name, visitor_phone, visitor_email, host_employee_id, purpose, time_in, client_ref)
       VALUES ('visitor', $1, $2, $3, $4, $5, now(), $6)
       RETURNING id, time_in`,
      [name, phone, email || null, host_employee_id, purpose || null, client_ref || null]
    );
