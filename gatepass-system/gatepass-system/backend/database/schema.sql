...
CREATE TABLE IF NOT EXISTS employees (
  ...
  phone          VARCHAR(30),
  active         BOOLEAN NOT NULL DEFAULT TRUE,
  source         VARCHAR(10) NOT NULL DEFAULT 'manual' CHECK (source IN ('manual', 'ldap')),
  ldap_synced_at TIMESTAMPTZ, -- last time an LDAP sync touched this row; NULL for manually-added records
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS visit_logs (
  ...
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
  client_ref        UUID UNIQUE, -- set by the kiosk when queuing an offline sign-in, so a retried
                                  -- sync after a dropped connection can't create a duplicate record
